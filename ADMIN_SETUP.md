...
- 持仓在后台手动录入（资产代码、数量、入场价格、杠杆倍数、方向、账户、是否公开），实时价格由服务端从 Gate 永续合约行情接口 `/api/v4/futures/{GATE_SETTLE}/tickers` 自动获取，默认结算币种 `usdt`。这是公开行情接口，不需要 Gate API Key 或任何密钥。
- 前台持仓卡片与详情弹窗按实时价格计算持仓市值，并以入场价格为基准、乘以杠杆倍数得到当前收益率（多仓看涨、空仓看跌），行情每 15 秒刷新一次。
- 首页顶部的 BTC、ETH Gate 永续价格标签每 15 秒从服务端刷新，浏览器不会接触 Gate 密钥。
