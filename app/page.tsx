...
type Holding = { id: string; asset: string; amount: number; value_usd: number | null; price_usd: number | null; direction: "long" | "short"; source: string; account: string; synced_at: string };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const key = process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY;
    if (!url || !key) return;
    fetch(url + "/rest/v1/portfolio_holdings?select=id,asset,amount,value_usd,price_usd,direction,source,account,synced_at&is_public=eq.true&order=value_usd.desc.nullslast", { headers: { apikey: key }, cache: "no-store" })
  ...
  return <div className={dark ? "site dark" : "site"} id="home">
    ...
    <main className="container content" id="articles">
      ...
      <aside className="sidebar">
        <button className="portfolio-card portfolio-card-button" onClick={() => setPortfolioOpen(true)} aria-label="查看详细持仓"><div className="portfolio-heading"><div><p className="portfolio-kicker">LIVE PORTFOLIO</p><h2>我的持仓</h2></div><span className="live-dot">● 实时 · 查看详情</span></div>{holdings.length ? <><div className="portfolio-total">{holdings.reduce((sum, item) => sum + (marketValue(item) ?? 0), 0).toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 2 })}</div><div className="holding-list">{holdings.slice(0, 6).map(item => { const value = marketValue(item); return <div className="holding-row" key={item.id}><strong>{item.asset}</strong><span>{item.amount.toLocaleString(undefined, { maximumFractionDigits: 8 })}</span><b>{value == null ? "—" : value.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 2 })}</b></div>; })}</div><small className="portfolio-updated">更新于 {holdingsUpdated ? new Date(holdingsUpdated).toLocaleString("zh-CN") : "—"}</small></> : <p className="portfolio-empty">持仓数据将在后台录入后显示。</p>}</button>
    ...
    {portfolioOpen && <div className="modal-backdrop" onClick={() => setPortfolioOpen(false)}><section className="portfolio-modal" role="dialog" aria-modal="true" aria-label="持仓详情" onClick={e => e.stopPropagation()}><button autoFocus className="modal-close" aria-label="关闭持仓详情" onClick={() => setPortfolioOpen(false)}>×</button><div className="portfolio-modal-head"><div><p className="portfolio-kicker">LIVE PORTFOLIO</p><h2>持仓详情</h2></div><span className="live-dot">● 实时价格 · 15 秒刷新</span></div>{holdings.length ? <div className="portfolio-detail-list">{holdings.map(item => { const direction = item.direction === "short" ? "short" : "long"; const mark = markPrice(item); const value = marketValue(item); return <div className="portfolio-detail" key={item.id}><div className="portfolio-detail-title"><div className="asset-heading"><strong>{item.asset}</strong><span className={`position-badge ${direction}`}>{direction === "short" ? "空仓" : "多仓"}</span></div><span>{item.source === "gate" ? `Gate · ${item.account}` : "手动持仓"}</span></div><div className="portfolio-metrics"><div><small>仓位方向</small><b className={`position-value ${direction}`}>{direction === "short" ? "空仓" : "多仓"}</b></div><div><small>持仓数量</small><b>{item.amount.toLocaleString(undefined, { maximumFractionDigits: 8 })}</b></div><div><small>实时价格</small><b>{mark == null ? "加载中…" : `$${mark.toLocaleString(undefined, { maximumFractionDigits: 4 })}`}</b></div><div><small>持仓市值</small><b>{value == null ? "—" : value.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 2 })}</b></div></div></div>; })}</div> : <p className="portfolio-empty">暂无公开持仓。</p>}<small className="portfolio-updated">最后更新：{holdingsUpdated ? new Date(holdingsUpdated).toLocaleString("zh-CN") : "—"}</small></section></div>}
  </div>;
}
