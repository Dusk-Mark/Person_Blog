...
export async function POST(request: Request) {
  ...
  const accounts = payload.filter((item): item is { contract: string; size: string; entry_price: string; mark_price: string; leverage: string } => Boolean(item && typeof item === "object" && "contract" in item && "size" in item && "entry_price" in item && "mark_price" in item)).map(item => ({ asset: item.contract.split("_")[0].toUpperCase(), amount: Math.abs(Number(item.size)), entry: Number(item.entry_price), mark: Number(item.mark_price), leverage: Number(item.leverage) })).filter(item => Number.isFinite(item.amount) && item.amount > 0);
  const syncedAt = new Date().toISOString();
  const rows = accounts.map(item => ({ owner_id: userData.user.id, source: "gate", account: `futures_${settle}`, asset: item.asset, amount: item.amount, price_usd: item.mark, entry_price_usd: Number.isFinite(item.entry) ? item.entry : null, mark_price_usd: item.mark, leverage: Number.isFinite(item.leverage) && item.leverage > 0 ? item.leverage : null, is_public: true, synced_at: syncedAt }));
